/* =============================================================
   IVIX_TV — desktop-only cinematic starfield (v7 clean)
   Clean version:
   • mobile: canvas is not created at all;
   • no CSS injection, no masking old styles from JS;
   • styles.css owns layout/z-index cleanup;
   • desktop: full opaque starfield canvas, so no split background;
   • stronger mouse parallax, subtle chaotic star drift, diffraction rays;
   • organic purple nebulae stay screen-fixed and slowly morph.
============================================================= */
(function () {
  "use strict";
  ...
  let layers = [], nebulae = [], glowSprites = {};
  let scrollFrac = 0, scrollDisp = 0;
  let mx = 0, my = 0, tmx = 0, tmy = 0;
  let lastT = 0, rafId = null, shoot = null, nextShoot = 0, mounted = false;

  const rand = (a, b) => a + Math.random() * (b - a);
  const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
  const lerp = (a, b, t) => a + (b - a) * t;

  function parseRgb(rgb) { return rgb.split(",").map((x) => parseFloat(x.trim())); }
  function mixRgb(a, b, t) {
    const ca = parseRgb(a), cb = parseRgb(b);
    return [Math.round(lerp(ca[0], cb[0], t)), Math.round(lerp(ca[1], cb[1], t)), Math.round(lerp(ca[2], cb[2], t))].join(", ");
  }
  ...
  function pickColorPair() {
    const i = Math.random() < CONFIG.warmChance ? 4 : ((Math.random() * (CONFIG.colorsTop.length - 1)) | 0);
    return { top: CONFIG.colorsTop[i], bottom: CONFIG.colorsBottom[i % CONFIG.colorsBottom.length] };
  }
  ...
  function buildLayer(cfg, layerIndex) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      stars.push({
        x: rand(-marginX, W + marginX), y: rand(-marginY, H + marginY), r,
        a: rand(cfg.a[0], cfg.a[1]), sp: rand(cfg.tw[0], cfg.tw[1]), ph: rand(0, Math.PI * 2),
        colorTop: pair.top, colorBottom: pair.bottom, glow: Math.random() < cfg.glow,
        ray: isBright, rayLen: rand(9, 28) * (0.7 + r * 0.34), rayAngle: rand(0, Math.PI),
        rayAlpha: rand(0.08, 0.26) * (layerIndex + 1) / CONFIG.layers.length,
        driftAmpX: rand(cfg.drift[0], cfg.drift[1]), driftAmpY: rand(cfg.drift[0], cfg.drift[1]) * 0.75,
        driftSpeedX: rand(0.070, 0.210), driftSpeedY: rand(0.060, 0.190),
        driftPhaseX: rand(0, Math.PI * 2), driftPhaseY: rand(0, Math.PI * 2),
      });
    }
    return { cfg, stars };
  }
  ...
  function rebuild() {
    ...
    glowSprites = {};
    CONFIG.colorsTop.concat(CONFIG.colorsBottom).forEach((color) => { glowSprites[color] = makeGlow(72, color); });
  }

  function resize() {
    if (isMobileLike()) { stopAndRemove(); return; }
  ...
  function drawSpaceBase() {
    ...
    sideA.addColorStop(0, "rgba(116, 50, 255, 0.20)"); sideA.addColorStop(0.45, "rgba(82, 32, 180, 0.08)"); sideA.addColorStop(1, "rgba(82, 32, 180, 0)");
    ctx.fillStyle = sideA; ctx.fillRect(0, 0, W, H);

    const sideB = ctx.createRadialGradient(W * 0.92, H * 0.18, 0, W * 0.92, H * 0.18, W * 0.55);
    sideB.addColorStop(0, "rgba(155, 70, 255, 0.16)"); sideB.addColorStop(0.42, "rgba(94, 36, 190, 0.07)"); sideB.addColorStop(1, "rgba(94, 36, 190, 0)");
    ctx.fillStyle = sideB; ctx.fillRect(0, 0, W, H);
  }
  ...
  function spawnShoot() {
    ...
    shoot = { x: fromLeft ? rand(-0.10, 0.32) * W : rand(0.68, 1.10) * W, y: rand(-0.08, 0.34) * H, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, life: 0, max: rand(0.72, 1.22), len: rand(130, 245), color: Math.random() < 0.28 ? CONFIG.colorsTop[4] : CONFIG.colorsTop[1] };
  }
  ...
  function drawOrganicNebula(n, tnow) {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      ctx.beginPath();
      ctx.moveTo(-0.82, yy);
      ctx.bezierCurveTo(-0.42, yy - 0.11 + shapeA * 0.03, -0.06, yy + 0.18 + shapeB * 0.03, 0.82, yy - 0.03);
      ctx.stroke();
    }
  ...
  function drawRay(px, py, star, color, alpha, tnow) {
    ...
    function strokeLine(theta, length, lineWidth, opacity) {
      const dx = Math.cos(theta) * length, dy = Math.sin(theta) * length;
      ...
      ctx.strokeStyle = grad; ctx.lineWidth = lineWidth; ctx.beginPath(); ctx.moveTo(px - dx, py - dy); ctx.lineTo(px + dx, py + dy); ctx.stroke();
    }
  ...
  function drawShoot(dt, tnow) {
    ...
    const ux = shoot.vx / speed, uy = shoot.vy / speed;
    ...
    ctx.strokeStyle = gradient; ctx.lineWidth = 1.45; ctx.lineCap = "round";
    ...
    ctx.beginPath(); ctx.arc(shoot.x, shoot.y, 1.3, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  ...
  function drawStars(tnow, isStatic) {
    ...
    for (const layer of layers) {
      ...
      for (const s of layer.stars) {
        const tw = isStatic ? 0.75 : (1 - cfg.twDepth) + cfg.twDepth * Math.pow(Math.sin(tnow * s.sp + s.ph) * 0.5 + 0.5, 1.65);
        const a = s.a * tw;
        if (a <= 0.010) continue;
        ...
        const px = s.x + offX + driftX, py = s.y + offY + driftY;
        ...
        if (s.r > 1.6) {
          ...
          ctx.beginPath(); ctx.arc(px, py, Math.max(0.35, s.r * 0.36), 0, Math.PI * 2); ctx.fill();
        }
      }
    }
    ctx.globalCompositeOperation = "source-over";
  ...
  function drawFrame(t, isStatic) {
    ...
    drawSpaceBase();
    if (CONFIG.nebulae) nebulae.forEach((n) => drawOrganicNebula(n, tnow));
    drawStars(tnow, isStatic);
    drawShoot(dt, tnow);
  }

  function frame(t) { drawFrame(t, false); lastT = t; rafId = requestAnimationFrame(frame); }
  function stopAndRemove() { if (rafId) cancelAnimationFrame(rafId); rafId = null; if (canvas.parentNode) canvas.parentNode.removeChild(canvas); mounted = false; }
  function onVisibility() { if (document.hidden) { if (rafId) cancelAnimationFrame(rafId); rafId = null; return; } if (!reduceMotion && !rafId && mounted) { lastT = performance.now(); rafId = requestAnimationFrame(frame); } }
  function debounce(fn, ms) { let id; return function () { clearTimeout(id); id = setTimeout(fn, ms); }; }

  function start() {
    if (isMobileLike()) { stopAndRemove(); return; }
    mount(); resize(); onScroll(); scrollDisp = scrollFrac;
    ...
    if (media && media.addEventListener) media.addEventListener("change", debouncedResize);
    if (window.ResizeObserver) { const ro = new ResizeObserver(debouncedResize); ro.observe(canvas); }
    if (reduceMotion) drawFrame(performance.now(), true); else { lastT = performance.now(); rafId = requestAnimationFrame(frame); }
  }
